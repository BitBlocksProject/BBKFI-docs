...
- **MasterChef farms** — stake BBKFI or LP tokens to earn BBKFI emissions. One contract runs many pools with `1 BBKFI/block` nominal emission, currently configured to ~`0.2 BBKFI/block` effective distribution to active pools.
...
- **Governance** sits above MasterChef, controlling pool weights and other protocol parameters. In practice, effective BBKFI emissions are tuned via allocPoints (including the ERTB limiter pool), while nominal emission stays at `1 BBKFI/block`.
